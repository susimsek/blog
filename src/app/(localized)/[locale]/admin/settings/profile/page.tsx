...
type AdminSettingsProfilePageProps = {
  params: Promise<{
    locale: string;
  }>;
};

export async function generateMetadata({ params }: AdminSettingsProfilePageProps): Promise<Metadata> {
...
export default async function AdminSettingsProfileRoute({ params }: Readonly<AdminSettingsProfilePageProps>) {
  const { locale } = await params;
  ...
  return (
    <RouteI18nProvider locale={locale} resources={resources}>
      <AdminAccountPage section="profile" />
    </RouteI18nProvider>
  );
}
