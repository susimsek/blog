...
type AdminSettingsSessionsPageProps = {
  params: Promise<{
    locale: string;
  }>;
};

export async function generateMetadata({ params }: AdminSettingsSessionsPageProps): Promise<Metadata> {
...
export default async function AdminSettingsSessionsRoute({ params }: Readonly<AdminSettingsSessionsPageProps>) {
  const { locale } = await params;
  ...
  return (
    <RouteI18nProvider locale={locale} resources={resources}>
      <AdminAccountPage section="sessions" />
    </RouteI18nProvider>
  );
}
