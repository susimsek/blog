...
type AdminSettingsContentPageProps = {
  params: Promise<{
    locale: string;
  }>;
};

export async function generateMetadata({ params }: AdminSettingsContentPageProps): Promise<Metadata> {
...
export default async function AdminSettingsContentRoute({ params }: Readonly<AdminSettingsContentPageProps>) {
  const { locale } = await params;
